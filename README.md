...
If you protect that Worker with [Cloudflare Access](https://developers.cloudflare.com/cloudflare-one/access-controls/applications/http-apps/authorization-cookie/validating-json/#cloudflare-workers-example) (recommended — Access rejects unauthorized requests at Cloudflare's edge, before your Worker code ever runs), tap **Open proxy to sign in**: it opens the proxy URL in a new tab so you can complete the Access login there. The app sends proxy requests with `credentials: "include"`, so the resulting `CF_Authorization` session cookie is forwarded on subsequent fetches automatically — no separate credentials to manage in the app itself.

Either way, add an Access policy that **bypasses `OPTIONS` requests** for this application. A CORS preflight carries no cookies by spec, so Access has nothing to validate on it — without a bypass, Access blocks the preflight with 403 before the browser ever attempts the real, authenticated request.
